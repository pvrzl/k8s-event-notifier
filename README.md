...
```sh
make docker-build docker-push IMG=<some-registry>/notifier:tag
```
...
```sh
make deploy IMG=<some-registry>/notifier:tag
```
...
```sh
make build-installer IMG=<some-registry>/notifier:tag
```
...
```sh
kubectl apply -f https://raw.githubusercontent.com/<org>/notifier/<tag or branch>/dist/install.yaml
```
